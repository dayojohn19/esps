...
class Sim():
    ...
    def receiveSMS(self):
        ...
        while rcvsms == None and ncount <= 10:
            print('                     receiving sms',ncount)
            sleep(1)
            ncount+=1
            rcvsms = self.uart.read()
            if rcvsms != None:
                break
        if rcvsms !=None: # if message is received
            print('             Found SMS')
            def delOldSms():            
                ...
                if "+CMGL:" in r:
                    ...
                    for l in ls:
                        if l.startswith("+CMGL:"):
                            ...
                            if isfirstdel == False:
                                print(f'   First Found: {i}')
                                isfirstdel = True
                                self.write(f"AT+CMGD={i},0\r\n", cmdPurpose="del oldst sms")
                                print('Deleted oldest sms')
                                latest = i
                                break
                # if latest:
                #     print(f' Found Last index {latest}')
                #     sleep(1)
                #     print(f'     Latest SMS: \n {lsms}')
                #     lsms = self.write(f'AT+CMGR={latest}\r\n')
                #     lsms = self.write(f'AT+CMGR=1\r\n')
                #     lsms = '\r\nOK\r\n\r\n+CMGR: "REC READ","+639765514253",,"25/02/19,23:47:52 +32"\r\n"if fundamental"\r\n\r\nOK'
                #     sms = lsms.split('\n')
                #     for sm in sms:
                #         if "+CMGR:" in sm:
                #             lines = sm.split('\n')
                ...
                return False
            delOldSms()
            self.checkTypeOfMessage(rcvsms)
    ...
    def checkTypeOfMessage(self,messageReceived):
        ...
        if textmsg >=0: # Receiving Text MEssage and write here
            ...
            self.record('sms.txt', toWriteData=f'[ {self.datetime()} ] - Receive -  {str(n)} ' )
            # self.record('sms.txt',f"{self.datetime()} - Sent - {message} \n") 
            try:
                n = n.split(" ", 1) # Check if has commands
