...
# clock_i2c = I2C(scl=Pin(14), sda=Pin(12), freq=100000)  # Example with GPIO14 and GPIO12 SDA=D6 SCL=D5
from configs.configs import clock_i2c
# devices = i2c.scan()
ds = DS3231(clock_i2c)
rtc = RTC()
rtc.datetime(ds.datetime())
def getTime():
    date = "{}/{}/{}" .format(rtc.datetime()[1], rtc.datetime()[2],rtc.datetime()[0]) 
    time =  "{}:{}:{}" .format(rtc.datetime()[4], rtc.datetime()[5],rtc.datetime()[6]) 
    # x = rtc.datetime()
    return [date,time]  