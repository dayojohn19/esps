...
class Battery():
    ...
    def motor(self):
        ...
        return self.battery(self.BatteryMotorPin,2,3898,min_reading=100)
    @property
    def module(self):
...
t = Train()
    ...
    # def feed(self):
    #     try:
    #         textWriter('feed.txt',f'Feed Done')
    #     except:pass
    #     self.ledlight.start(feedblink,10)
    #     time.sleep(1.5)
    #     self.feeder.go 
    #     time.sleep(3)
    #     ledlight.stop()

    #     ledlight.start(500,2)

    # def start(self,train=1, seconds=None):
    #     try:
    #         # ic.active(False)
    #         print('Add func to')
    #         print('Turning Network OFF')
    #         print('Turning  =card off')
    #     except Exception as e:
            ...
            if wake.reason == 'deepsleep':
                ...
                try:
                    ...
                    try:
                        ts = sim.datetime()
                        try:
                            sim.sendSMS(message=f'{ts} Device Successfully Run Recursions ')
                        except Exception as e:
